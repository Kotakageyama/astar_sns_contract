...
mod astar_sns_contract {
    ...
    impl AstarSnsContract {
        ...
        pub fn get_general_post(&self, num: u128) -> Vec<Post> {
            let general_post_list: Vec<Post> = self.get_general_post_fn(num);
            general_post_list
        }
        ...
        pub fn get_individual_post(&self, num: u128, account_id: AccountId) -> Vec<Post> {
            let individual_post_list: Vec<Post> = self.get_individual_post_fn(num, account_id);
        ...
        // 指定のアカウントの投稿に対するいいねの総数を取得
        #[ink(message)]
        pub fn get_total_liks(&self, account_id: AccountId) -> u128 {
            let num_of_liks: u128 = self.get_total_likes_fn(account_id);
            num_of_liks
        }
        ...
        pub fn balance_of(&self, account_id: AccountId) -> u128 {
            let asset: u128 = self.balance_of_fn(account_id);
            asset
        }
        ...
        pub fn distribute_refer_likes(&mut self) {
            ...
            let asset: u128 = self.balance_of_fn(caller);
